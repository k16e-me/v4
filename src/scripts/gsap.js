...
export default function _gsap() {
    ...
    const outBottom = { autoAlpha: 0, y: 48, ease: 'back.in' }

    const
        moveX = (el, props) => tl().to(el, { ...props }),
        slideOut = (el, props) => tl().to(el, { outBottom, ...props }),
        slideOutTop = (el, props) => tl().to(el, { autoAlpha: 0, y: -48, ease: 'back.in', ...props }),
    ...
    return { moveX, slideOut, slideIn, slideTop, slideBottom, slideOff, on, off, slideOutTop }
}